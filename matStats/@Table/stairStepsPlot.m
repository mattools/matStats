function varargout = stairStepsPlot(varargin)
% Plot the content of a column as stairs.
%
%   output = stairStepsPlot(input)
%
%   Example
%     % stairs plot of a sinus curve
%     t = linspace(0, 2*pi, 100)';
%     data = [t cos(t) sin(t)];
%     tab = Table(data, {'t', 'cos(t)', 'sin(t)'});
%     figure; stairStepsPlot(tab(:,1), tab(:,2:3));
%
%   See also
%     plot, linePlot, stemPlot, barPlot
%
...
if isempty(tabX)
    % plot(Y)
    h = stairs(ax, tabY.Data, varargin{:});
    
else
    % plot(X, Y)
    h = stairs(ax, xData, tabY.Data, varargin{:});
