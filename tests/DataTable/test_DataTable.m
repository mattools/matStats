function tests = test_DataTable
%TESTTABLE  One-line description here, please.
%
%   output = testDataTable(input)
%
%   Example
%   testDataTable
%
...
assertTrue(testCase, isa(tab, 'DataTable'));
...
tab = DataTable({set.Origin, set.MPG, set.Cylinders, set.Displacement});
...
tab = DataTable(array, names);
assertEqual(testCase, names, tab.ColNames);

tab = DataTable(array, 'colNames', names);
assertEqual(testCase, names, tab.ColNames);
...
tab = DataTable(array, 'rowNames', names);
...
tab = DataTable(t, xt, yt, 'rowNames', rowNames);
assertEqual(testCase, [4 3], size(tab));
assertEqual(testCase, 'xt', tab.ColNames{2});
